...
mod handlers {
    ...
    pub async fn root<S>(_state: State<ServerState<S>>) -> impl IntoResponse
    where
        S: Store + Clone + Send + Sync + 'static,
    {
    ...
    pub async fn command<S>(
        State(mut _state): State<ServerState<S>>,
        ...
        S: Store + Clone + Send + Sync + 'static,
    {
        match _state
            .store
            .store_operation(command_type.clone(), _command, command_type.clone())
            .await
        {
    ...
    pub async fn query<S>(
        ...
        State(_state): State<ServerState<S>>,
    ) -> impl IntoResponse
    where
        S: Store + Clone + Send + Sync + 'static,
    {
