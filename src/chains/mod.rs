...
impl ProcessingChain {
    pub(crate) fn new(links: Vec<Arc<dyn ChainLink>>) -> Self {
        ...
        let response_size_histogram = meter
            .u64_histogram("cqrl_processing_chain_response_size")
            .build();
        let requests = meter.u64_counter("cqrl_processing_chain_requests").build();
        Self {
            links,
            response_size_histogram: Arc::new(response_size_histogram),
    ...
    fn call(
        ...
        Box::pin(async move {
            ...
            if context.get(RESPONSE_DATA_KEY).is_some() {
                let Some(response_data) = context.get(RESPONSE_DATA_KEY) else {
                    ...
                    response_size_histogram
                        .record(content.len() as u64, &[KeyValue::new("status", "500")]);
                    return HttpResponse::InternalServerError().body(content);
                };
            ...
            if let Some(command_header) = context.get(RESPONSE_HEADER_COMMAND_KEY) {
                builder = builder
                    .append_header(("X-Command-Id", command_header.to_string()))
                    .take();
            }
